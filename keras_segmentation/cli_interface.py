...
from .predict import predict, predict_multiple, predict_video, evaluate, best
...
def best_model_action(command_parser):

    parser = command_parser.add_parser('best_mIU_model')
    parser.add_argument("--images_path", type=str, required=True)
    parser.add_argument("--segs_path", type=str, required=True)
    parser.add_argument("--checkpoints_path", type=str, required=True)
...
def main():
    ...
    best_model_action(command_parser)
