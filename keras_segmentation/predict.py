...
def get_model(checkpoints_path, checkpoint):
    from .models.all_models import model_from_name
    assert (os.path.isfile(checkpoints_path+"_config.json")
            ), "Checkpoint not found."
    model_config = json.loads(
...
def evaluate(model=None, inp_images=None, annotations=None,
    ...
    for inp, ann in tqdm(zip(inp_images, annotations)):
        ...
        tp0 = np.zeros(model.n_classes)
        tn0 = np.zeros(model.n_classes)
        fp0 = np.zeros(model.n_classes)
        fn0 = np.zeros(model.n_classes)
        ...
        for cl_i in range(model.n_classes):
            ...
            # single
            tp0[cl_i] += np.sum((pr == cl_i) * (gt == cl_i))
            fp0[cl_i] += np.sum((pr == cl_i) * ((gt != cl_i)))
            fn0[cl_i] += np.sum((pr != cl_i) * ((gt == cl_i)))
            tn0[cl_i] += np.sum((pr != cl_i) * ((gt != cl_i)))
            #single

        cl = tp0 / (tp0 + fp0 + fn0 + 0.000000000001)
        mean_IU0 = np.mean(cl)
        print(f'mean_IU: {mean_IU0}')
        if mean_IU0 > best_mIU:
            best_mIU = mean_IU0
            max_idx = idx
        idx +=1
    print(f'best_IU: {best_mIU}| ix = {max_idx}')
...
def best(model=None, inp_images=None, annotations=None,
    ...
    for name in all_checkpoint_files:
        model = get_model(checkpoints_path, name)
        tp = np.zeros(model.n_classes)
        tn = np.zeros(model.n_classes)
        fp = np.zeros(model.n_classes)
        fn = np.zeros(model.n_classes)
